...
// Exclui os automáticos de mensalidade (referencia_tipo = 'mensalidade'), que já
// estão somados acima direto da tabela mensalidades (mensPagas). Sem esse filtro,
// cada mensalidade paga entrava duas vezes no total.
$stRecLanc = $pdo->prepare("
    ...
    WHERE competencia IN ($inPlaceholders) AND tipo = 'receita'
      AND (referencia_tipo IS NULL OR referencia_tipo <> 'mensalidade')
");
...
// Exclui os automáticos de parcela de dívida (referencia_tipo = 'parcela_divida'),
// que já estão somados acima direto da tabela parcelas_dividas (parcelasPagas).
$stDespLanc = $pdo->prepare("
    SELECT COALESCE(SUM(valor), 0) FROM lancamentos_financeiros
    WHERE competencia IN ($inPlaceholders) AND tipo = 'despesa'
      AND (referencia_tipo IS NULL OR referencia_tipo <> 'parcela_divida')
");
