...
if ($pagamentoId <= 0) {
    ...
    echo json_encode(['success' => false, 'message' => 'ID de pagamento inválido.']);
    exit;
}
...
$check = $pdo->prepare("SELECT id FROM professor_pagamentos WHERE id = ?");
$check->execute([$pagamentoId]);
if (!$check->fetch()) {
...
$uploadDir = dirname(__FILE__, 3) . '/uploads/pagamentos_professor/' . $pagamentoId . '/';
if (!is_dir($uploadDir)) {
...
for ($i = 0; $i < $count; $i++) {
    ...
    $caminho = 'uploads/pagamentos_professor/' . $pagamentoId . '/' . $filename;
    $pdo->prepare("
        INSERT INTO professor_pagamento_anexos (pagamento_id, nome_original, caminho, tipo_mime, tamanho)
        VALUES (?, ?, ?, ?, ?)
    ")->execute([$pagamentoId, $files['name'][$i], $caminho, $mime, $files['size'][$i]]);
