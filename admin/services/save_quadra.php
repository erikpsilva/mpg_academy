...
try {
    ...
    foreach ($horarios as $h) {
        ...
        $s->execute([$quadraId, $dia, $inicio, $fim]);
        $horarioIds[] = (int) $pdo->lastInsertId();
    }

    // Insere turmas e seus horários
    foreach ($turmas as $t) {
        ...
        foreach ($t['horario_indices'] ?? [] as $idx) {
            ...
            if (isset($horarioIds[$idx])) {
                $s = $pdo->prepare("INSERT INTO turma_horarios (turma_id, horario_id) VALUES (?, ?)");
                $s->execute([$turmaId, $horarioIds[$idx]]);
            }
        }
    }
    ...
    echo json_encode(['success' => true, 'message' => 'Quadra cadastrada com sucesso!', 'id' => $quadraId]);

} catch (Exception $e) {
    $pdo->rollBack();
    error_log('[save_quadra] ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erro interno ao salvar quadra.']);
}
