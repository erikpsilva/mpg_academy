<?php
/**
 * CRON — Lembrete aula experimental na véspera (1 dia antes)
 * Configurar no cPanel: diariamente às 18:00
 * Comando: php /home/SEU_USUARIO/public_html/mpg_academy/cron/wpp_lembrete_amanha.php
 */
...
avisoCronExigeAtivo($pdo, 'aviso_teste_vespera', 'Lembretes véspera (amanhã)');

$alvo = (new DateTime('now', new DateTimeZone('America/Sao_Paulo')))->modify('+1 day')->format('Y-m-d');
...
$jaEnviado = $pdo->prepare("SELECT id FROM lembrete_teste_log WHERE aula_experimental_id = ? AND tipo = 'amanha' AND DATE(enviado_em) = CURDATE() LIMIT 1");
$logInsert = $pdo->prepare("INSERT INTO lembrete_teste_log (aula_experimental_id, tipo) VALUES (?, 'amanha')");
...
foreach ($rows as $r) {
    ...
    wppAulaTesteLembrete($r, 'amanha');
    $logInsert->execute([$r['id']]);
    $enviados++;
}

echo "Lembretes véspera (amanhã): {$enviados} enviados.\n";
